...
def test_main_writes_pre_phase_a_yaml_connectivity_svg(scratch_dir: Path) -> None:
    ...
    # IC1 PIN_1 seg3: B-coordinate-system formula with A pointing down (0,-1).
    # angle=90 → B points right (+x); offset_u=-1.6 along B → dx=-1.6;
    # edge_left → signed_v=+(W/2+c)=+(1.27/2+0.15)=0.785 along B-left (+y) → dy=+0.785.
    ...
    assert seg3_start["x"] - pin1_node["x"] == pytest.approx(-1.6, abs=0.15)
    assert seg3_start["y"] - pin1_node["y"] == pytest.approx(0.785, abs=0.15)
    # angle=90 means seg3 runs rightward from its launch point.
    ...
    # align_left → offset_u=0; edge_front → signed_v=0; anchor = node center.
    ...
    # C1/R1 are two-pin passives: seg4 → C1.PIN_1,R1.PIN_1, seg5 starts at C1.PIN_2,R1.PIN_2.
    seg5_render = by_edge["IC1_pin1_seg5"]["render_endpoint_positions_mm"]
    c1r1_pin1 = by_edge["IC1_pin1_seg4"]["render_endpoint_positions_mm"][
        "C1.PIN_1,R1.PIN_1"
    ]
    c1r1_pin2 = seg5_render["C1.PIN_2,R1.PIN_2"]
    c1r1_span = math.hypot(
        c1r1_pin2["x"] - c1r1_pin1["x"],
        c1r1_pin2["y"] - c1r1_pin1["y"],
    )
    # Pin pitch of C1/R1 should stay within package scale
    assert c1r1_span < 3.0
    assert "C2.PIN_1" in seg5_render
    r3_seg4 = by_edge["IC1_pin2_seg4"]["render_endpoint_positions_mm"]
    r3_chain = by_edge["IC1_pin2_seg5"]["render_endpoint_positions_mm"]
    # R3 two-pin passive: PIN_1 and PIN_2 should be laterally separated
